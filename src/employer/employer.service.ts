...
export class EmployerService {
    ...
    async getBatch(batchId: string, employerId: string) {
        ...
        const uploads = JSON.parse(String(batch.uploads)).map((id: string) => new Types.ObjectId(id));
    ...
    async uploadBatchJD(batchId: string, employerId: string, file: Express.Multer.File) {
        const currentBatch = await this.prismaService.batch.findUniqueOrThrow({
            where: {
                id: batchId,
                employerId
